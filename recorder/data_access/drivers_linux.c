...
int getDriver()
{
    ...
    if ((fp =  fopen("/proc/modules","r")) == 0) {
        DEBUGMSGTL(("recorder:log", "Unable to open /proc/modules\n"));
        return 0;
    }
    ...
    if ((drivers = malloc(modcount * sizeof(drv_entry))) == NULL) {
        DEBUGMSGTL(("recorder:log", "Unable to malloc() %ld bytes\n", 
    ...
    while (fgets(buffer, sizeof(buffer), fp) > 0) {
        ...
        if (found != NULL) {
            ...
            if (stat(drivers[i]->filename, &buf) == 0 ) {
            ...
            DEBUGMSGTL(("recorder:log", "i = %d ", i));
            DEBUGMSGTL(("recorder:log", "module = %s ", drivers[i]->name));
            DEBUGMSGTL(("recorder:log", "filename = %s ", drivers[i]->filename));
            DEBUGMSGTL(("recorder:log", "timestamp = %s \n", drivers[i]->timestamp));
            DEBUGMSGTL(("recorder:log", "version = %s\n", drivers[i]->version));
