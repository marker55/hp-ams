...
int getService()
{
    ...
    for (i = 0; i <proccount; i++ ) {
        ...
        DEBUGMSGTL(("recorder:log","service proc  status %s\n",procname));
        ...
        DEBUGMSGTL(("recorder:log","service proc pid = %d \n", pid));
        if (values[PROC_TGID] != NULL)
            tgid = atoi(values[PROC_TGID]);
        DEBUGMSGTL(("recorder:log","service proc tgid = %d \n", tgid));
        if (tgid != pid) {
            close(procFd);
            free(proclist[i]);
            continue;
        ...
        DEBUGMSGTL(("recorder:log","service proc name %s\n",  services[svccount]->name));
        /* not a thread */
        ...
        DEBUGMSGTL(("recorder:log","service file name %s\n", 
