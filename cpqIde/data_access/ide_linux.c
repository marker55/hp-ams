    ...
    for (i=0; i < NumIdeHost; i++) {
        ...
        *pbuffer = '\0';
        if ((sbuffer = strrchr(lbuffer, '/')) == NULL) 
            continue;
        sbuffer++;
        sscanf(sbuffer, "%4hx:%2hhx:%2hhx.%1hhx",
                &domain, &bus, &dev, &func);
        it = CONTAINER_ITERATOR(container);
        ctlr = ITERATOR_FIRST(it);

        while ( ctlr != NULL ) {
            if ((ctlr->domain == domain) && (ctlr->bus == bus) && 
                    (ctlr->dev == dev) && (ctlr->func == func))  
                break;
...
int netsnmp_arch_idedisk_container_load(netsnmp_container* container)
{
    ...
    cpqIdeControllerTable_entry *prev_cntlr = NULL;
    ...
    DEBUGMSGTL(("idedisk:container:load", "SATA disk count = %d\n", NumIdeDisk));
    ...
    cntlr = ITERATOR_FIRST( it );
        ...
        cntlr->cpqIdeControllerOverallCondition =
            ...
            if ((cntlr->domain != domain) || (cntlr->bus != bus) ||
                    (cntlr->dev != dev) || (cntlr->func != func)) {
